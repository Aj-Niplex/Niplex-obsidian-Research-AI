...
The optional private helper repository is [Aj-Niplex/niplex-obsidian-helper](https://github.com/Aj-Niplex/niplex-obsidian-helper). Its Marketplace section accepts a unique five-character code such as `RSH01`, fetches the private [Niplex-Obsidian-skills](https://github.com/Aj-Niplex/Niplex-Obsidian-skills) catalogue, verifies a digest, previews the instruction-only manifest, and requires explicit installation approval. The helper never reuses provider keys. After relaunch, the main plugin reads validated packages from `NIPLEX-OBSIDIAN/Skills/` and applies only allowlisted bounded settings patches plus additive untrusted guidance. Skills cannot run scripts, replace the protected prompt, acquire keys, or bypass write approval. The full protocol is in `docs/skill-marketplace-design.md`.
