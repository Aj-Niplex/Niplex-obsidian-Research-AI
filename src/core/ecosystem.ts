...
export function normalizeEcosystemContribution(
	...
	const text = contribution.text.slice(0, Math.min(Math.max(request.maxChars, 200), 12000));
	if (!text.trim()) return null;
	return {
		extensionId: contribution.extensionId.slice(0, 100),
		label: contribution.label.slice(0, 120),
		text,
		dataClasses: contribution.dataClasses.slice(0, request.maxItems),
		provenance: contribution.provenance.slice(0, request.maxItems).map((item) => ({
			label: item.label.slice(0, 180),
			kind: item.kind,
			...(item.path ? { path: item.path.slice(0, 180) } : {}),
			...(item.relation ? { relation: item.relation.slice(0, 120) } : {}),
		})),
		truncated: contribution.truncated || contribution.text.length > text.length,
