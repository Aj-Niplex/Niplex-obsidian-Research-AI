...
export default class AgenticResearchPlugin extends Plugin implements SettingsHost, AgentViewHost, CompanionInstallHost {
	...
	async runAgent(
		...
		const hasSelectedSkill = selectedSkillCodes.some((code) => /^[A-Z0-9]{5}$/.test(code.trim().toUpperCase()));
			...
			const requestsKnowledgePlacement = /\b(put|place|save|store|organize|section|folder|favorite|favourite|music|song|youtube|video|asset|note|moc|map)\b/.test(query);
		const isFirstTurn = !history.some((message) => message.role === "user");
			...
			if (/^continue\b/i.test(prompt.trim())) hints.push("Continuation rule: reuse the bounded evidence already present in this conversation. Do not reread the same file or MOC merely to refresh it; only request a non-overlapping window or a genuinely different relevant file when the existing evidence cannot answer the request.");
			if (requestsKnowledgePlacement) hints.push("Destination rule: Assets/ is only for binary attachments such as mp3, mp4, png, and pdf. Never create or append Markdown knowledge notes or MOCs under Assets/. Put Markdown in a user-facing notes/category folder, create missing parent folders, and link binary files with Obsidian embeds such as ![[Assets/file.ext]]. If the user names a category such as Music, Favorites, or YouTube, use a clearly named Markdown category note (for example Music.md or Favorites/Music.md) and update the relevant MOC separately.");
