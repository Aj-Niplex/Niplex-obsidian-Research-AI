...
Aj-Niplex MCP was used to inspect the reference repositories and official Gemini and Agnes documentation, then to create the private `Aj-Niplex/Dev-obsidian-agentic-research` repository. The project’s provider adapters remain direct mobile-safe HTTP adapters because an Obsidian plugin cannot automatically inherit the Manus session’s MCP credentials or server configuration.
