...
The composer includes separate provider and model selectors. The model selector is populated from the selected provider’s live catalogue when the API key is available, while the configured model remains available if catalogue refresh is unavailable. Changing the selector updates the active provider/model for the next run and persists the preference. If a model is rate-limited, the runtime records a one-minute cooldown for that provider/model, avoids it during the cooldown, tries another available chat model from the same provider, and reports the switch in the compact activity cards. Obsolete model IDs are quarantined longer to avoid repeating stale-model errors. It never silently changes from Gemini to Agnes or vice versa.
...
The plugin stores a bounded local diagnostic history for provider failures, model cooldowns, model switches, and MOC failures. The share view redacts API-key-like values and vault paths and excludes prompts, model responses, and note excerpts. Users can share the text through the mobile share sheet or copy it when the browser clipboard API is available.
